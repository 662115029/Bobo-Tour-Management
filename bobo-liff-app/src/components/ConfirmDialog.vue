            ...
            <transition name="dialog-pop" appear>
                <div v-if="isOpen" class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                    <div class="px-6 pt-6 pb-2 flex flex-col items-center text-center">
                        ...
                        <h3 class="text-body font-bold text-gray-900 mb-1.5">{{ title }}</h3>
                        <p class="text-caption text-gray-500 leading-relaxed">{{ message }}</p>
                    </div>
                    <div class="flex gap-2 px-6 pb-6 pt-4">
                        <button @click="handleCancel"
                            class="flex-1 px-4 py-2.5 text-caption font-semibold rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Cancel</button>
                        <button @click="handleConfirm"
                            class="flex-1 px-4 py-2.5 text-caption font-semibold rounded-xl bg-red-600 text-white hover:bg-red-700 transition shadow-sm">Continue</button>
                    </div>
                </div>
            </transition>
        </div>
