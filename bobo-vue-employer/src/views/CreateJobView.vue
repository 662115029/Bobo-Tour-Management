  ...
  <div class="max-w-4xl mx-auto p-4 pb-20">
    ...
    <form @submit.prevent="submitJob">
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        <h2 class="text-lg font-semibold mb-3 text-blue-600 border-b pb-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium mb-1"
              >Number of Seats Required *</label
            >
            <input
              v-model.number="form.job_required_seat"
              type="number"
              min="1"
              max="13"
              required
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              v-model="form.job_required_vehicle_type"
              required
              class="w-full p-2 border rounded"
            >
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div
          v-for="(item, idx) in form.job_itineraries"
          :key="idx"
        ...
        <button
          type="button"
          @click="addItinerary"
          class="mt-2 px-4 py-2 bg-green-100 text-green-700 rounded hover:bg-green-200"
        >
          + Add Stop
        </button>
      </section>
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div
          v-for="(pickup, idx) in form.job_pickups"
          :key="idx"
        ...
        >
          ...
          <div>
            <label class="block text-xs text-gray-500"
              >Group / Customer Name</label
            >
            <input
              v-model="pickup.pickup_location"
              type="text"
              class="w-full p-2 border rounded text-sm"
              placeholder="Group / Customer Name"
            />
          </div>
          <div>
            <label class="block text-xs text-gray-500">Pickup Time</label>
            <input
              v-model="pickup.pickup_time"
              type="time"
              class="w-full p-2 border rounded text-sm"
            />
          </div>
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div
          v-for="(customer, idx) in form.job_customers"
          :key="idx"
        ...
        <div class="mt-3 flex justify-between items-center border-t pt-3">
          ...
          <div class="text-sm font-medium">
            ...
            <span class="text-lg font-bold text-blue-600"
              >{{ form.job_customers.length }} passenger(s)</span
            >
          </div>
        </div>
      </section>
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div
          v-for="(inc, idx) in form.job_inclusions"
          :key="idx"
          class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-2 items-end"
        >
          <div>
        ...
        <button
          type="button"
          @click="addInclusion"
          class="mt-2 mr-2 px-4 py-2 bg-green-100 text-green-700 rounded hover:bg-green-200"
        >
          + Add
        </button>
      </section>
      ...
      <section class="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <div
          v-for="(fee, idx) in form.job_entrance_fees"
          :key="idx"
          class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-2 items-end"
        >
          <div>
          ...
          <div>
            ...
            <input
              v-model.number="fee.foreigner_price"
              type="number"
              class="w-full p-2 border rounded text-sm"
            />
          </div>
      ...
      <!-- Submit Button -->
      <div
        class="fixed bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg"
      >
...
const addPickup = () =>
  form.job_pickups.push({
    hotel_name: "",
    pickup_location: "",
    pickup_time: "",
    sequence: form.job_pickups.length + 1,
  });
const removePickup = (idx) => form.job_pickups.splice(idx, 1);
const addCustomer = () =>
  form.job_customers.push({ customer_name: "", note: "" });
const removeCustomer = (idx) => form.job_customers.splice(idx, 1);
const addInclusion = () =>
  form.job_inclusions.push({
    inclusion_type: "INCLUSION",
    description: "",
    sequence: form.job_inclusions.length + 1,
  });
const removeInclusion = (idx) => form.job_inclusions.splice(idx, 1);
const addEntranceFee = () =>
  form.job_entrance_fees.push({
    place_name: "",
    thai_price: 0,
    foreigner_price: 0,
    sequence: form.job_entrance_fees.length + 1,
  });
...
const submitJob = async () => {
  try {
    ...
    if (res.ok) {
      ...
      alert("An error occurred: " + (err.error || err.message || "unknown"));
    }
  } catch (e) {
