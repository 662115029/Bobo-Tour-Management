  ...
  <div v-if="data" class="modal-overlay" @click.self="$emit('close')">
    <div class="w-[400px] rounded-2xl bg-white shadow-[0_16px_56px_rgba(0,0,0,0.18)] flex flex-col max-h-[90vh] overflow-y-auto">
      ...
      <div class="relative flex items-center justify-center px-5 py-3.5 border-b border-[#f0f0f0]">
        <span class="text-meta font-bold uppercase tracking-widest text-[#1a73e8]">Job</span>
        <button
        ...
        <div class="relative px-5 pt-5 pb-4">
          <div class="flex items-start gap-3.5">
            ...
            <div class="flex-1 min-w-0 pt-0.5">
              <div class="text-body font-bold text-[#111] leading-snug">{{ data.job_title }}</div>
              <div class="text-caption mt-1 line-clamp-2 leading-relaxed" :class="data.job_description ? 'text-[#777]' : 'text-[#bbb] italic'">
          ...
          <div class="flex items-center justify-between mt-3.5">
            ...
            <span v-if="data.job_price" class="text-body font-bold text-[#1a73e8]">
      ...
      <div class="px-5 py-4 flex flex-col gap-4">
        ...
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-full flex items-center justify-center text-caption font-bold shrink-0 ring-2 ring-[#eee]"
            :style="avatarStyle(data.em_id, data.company)">{{ initials2(data.company) }}</div>
          <div class="min-w-0">
            <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#bbb]">Employer</div>
            <div class="text-caption font-semibold text-[#222] truncate mt-0.5">{{ data.company || '-' }}</div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-3 flex-1">
            <div>
              <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#2e7d32]">Start</div>
              <div class="text-caption font-semibold text-[#111] mt-0.5">{{ formatDate(data.job_start_date) || '-' }}</div>
            </div>
            <svg width="18" height="18" fill="none" stroke="#ccc" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14M13 6l6 6-6 6"/>
            </svg>
            <div>
              <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#c62828]">End</div>
              <div class="text-caption font-semibold text-[#111] mt-0.5">{{ formatDate(data.job_end_date) || '-' }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#bbb]">Vehicle</div>
              <div class="text-caption font-semibold text-[#222] mt-0.5">{{ data.job_required_vehicle_type || '-' }}</div>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-xl bg-[#f5f5f5] flex items-center justify-center shrink-0">
            ...
            <div>
              <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#bbb]">Seats</div>
              <div class="text-caption font-semibold text-[#222] mt-0.5">{{ data.job_required_seat || '-' }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div v-if="data.selected_driver"
            class="w-8 h-8 rounded-full flex items-center justify-center text-caption font-bold shrink-0 ring-2 ring-[#eee]"
          ...
          <div class="flex-1 min-w-0">
            <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#bbb]">Driver</div>
            <div class="text-caption font-semibold mt-0.5 truncate"
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-meta font-semibold uppercase tracking-[0.08em] text-[#bbb]">Required Languages</div>
            <div v-if="data.languages?.length" class="flex flex-wrap gap-1.5 mt-1.5">
              <span v-for="lang in data.languages" :key="lang"
                class="text-meta font-semibold bg-[#e8f0fe] text-[#1a73e8] px-2.5 py-0.5 rounded-full">{{ lang }}</span>
            </div>
            <div v-else class="text-caption text-[#bbb] italic mt-0.5">No requirement</div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 border-t border-[#f0f0f0] bg-[#fafafa]">
        <div class="flex flex-col items-center text-center px-4 py-3 border-r border-[#f0f0f0]">
          ...
          <div class="text-caption font-medium text-[#999] mt-0.5">{{ formatDateTime(data.job_created_at) }}</div>
        </div>
        <div class="flex flex-col items-center text-center px-4 py-3">
          ...
          <div class="text-caption font-medium text-[#999] mt-0.5">{{ formatDateTime(data.job_updated_at) }}</div>
        </div>
      </div>
      ...
      <div class="px-5 py-4">
        <button
          class="w-full py-3 bg-[#111] text-white text-caption font-semibold rounded-xl border-none cursor-pointer transition-colors hover:bg-[#2a2a2a] active:scale-[0.98] flex items-center justify-center gap-1.5"
