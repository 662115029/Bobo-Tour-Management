<template>
  ...
  >
    ...
    <div class="flex flex-col gap-0.5 border-t border-white/[0.08] py-2.5" :class="isOpen ? 'px-2' : 'px-1'">
      ...
      <div
        class="mb-1 flex cursor-pointer items-center gap-2.5 rounded-lg p-2.5 transition-colors hover:bg-white/[0.07]"
      ...
      >
        ...
        <div v-if="isOpen" class="flex min-w-0 flex-col overflow-hidden">
          <span class="truncate text-[13px] font-semibold text-white">{{ adminName || 'Admin' }}</span>
          <span class="truncate text-[11px] text-white/40">Administrator</span>
        </div>
      </div>
