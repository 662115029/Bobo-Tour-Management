<template>
  <div>
                            ...
                            : actionFilter === "BAN_USER" ? "BAN ▼"
                              : actionFilter === "UNBAN_USER" ? "UNBAN ▼"
                                : actionFilter === "DELETE" ? "DELETE ▼"
                                  ...
                                  : actionFilter === "DELETE_USER" ? "DELETE USER ▼"
    ...
    <div v-if="showActionDropdown" class="col-dropdown" :style="actionDropdownStyle">
      ...
      <button class="col-dropdown-item" @click="setActionFilter('DELETE_USER')">Delete User</button>
    </div>
    ...
    <div v-else-if="deletedJobModal" class="modal-overlay" @click.self="deletedJobModal = null">
      <div class="w-[380px] rounded-2xl bg-white shadow-[0_16px_56px_rgba(0,0,0,0.18)] flex flex-col overflow-hidden">
        ...
        <div class="flex flex-col items-center px-6 pt-7 pb-5">
          ...
          <div class="text-[17px] font-bold text-[#111] text-center leading-snug">This {{ deletedLabel.toLowerCase() }} has been deleted</div>
          <div class="text-[13px] text-[#999] text-center mt-1.5 leading-relaxed">This {{ deletedLabel.toLowerCase() }} no longer exists in the system.</div>
        </div>
        ...
        <div class="px-5 py-4 flex flex-col gap-3">
          <div class="flex items-center justify-between bg-[#fafafa] rounded-xl px-4 py-3">
            <span class="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#bbb]">{{ deletedLabel }} Name</span>
...
const deletedLabel = computed(() => ({ FREELANCER: 'Freelancer', EMPLOYER: 'Employer' }[(deletedJobModal.value?.target_type || '').toUpperCase()] || 'Job'));
...
const openTargetModal = async (log) => {
  ...
  if ((type === 'JOB' && (action === 'DELETE_JOB' || action === 'DELETE')) || action === 'DELETE_USER') {
