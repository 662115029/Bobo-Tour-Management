  ...
  <div>
    ...
    <div class="table-container">
      <table class="table jobs-table">
        <thead>
          <tr>
            <th class="th-sortable" :class="{ 'th-active': titleSort }" style="width: 24%" @click="cycleSort('title')">
            ...
            <th class="th-sortable" :class="{ 'th-active': companySort }" style="width: 18%" @click="cycleSort('company')">
              <span class="th-inner">EMPLOYER
                <span class="sort-label">
            ...
            <th class="th-sortable" :class="{ 'th-active': priceSort }" style="width: 8%" @click="cycleSort('price')">
            ...
            <th class="th-sortable" :class="{ 'th-active': startSort }" style="width: 12%" @click="cycleSort('start')">
              <span class="th-inner">START DATE
                <span class="sort-label">
                  <span v-if="!startSort" class="sort-label-dim">⇅</span>
                  <span v-else-if="startSort === 'asc'" class="sort-label-active">↑01→31</span>
                  <span v-else class="sort-label-active">↓31→01</span>
                </span>
              </span>
            </th>
            <th class="th-sortable" :class="{ 'th-active': dateSort }" style="width: 16%; position: relative;" @click="cycleSort('date')">
              ...
              <button v-if="titleSort || companySort || priceSort || statusFilter !== 'All' || dateSort === 'asc' || startSort"
...
const saveFilters = () => {
  ...
  localStorage.setItem("jobs_startSort", startSort.value)
}

const cycleSort = (key) => {
  const current = { title: titleSort, company: companySort, price: priceSort, date: dateSort, start: startSort }[key]
  const next = current.value === 'desc' ? 'asc' : 'desc'
  titleSort.value = ""; companySort.value = ""; priceSort.value = ""; dateSort.value = ""; startSort.value = ""
  current.value = next
  saveFilters()
  page.value = 1
  fetchJobs()
...
const fetchJobs = async () => {
  ...
  try {
    ...
    if (startSort.value) { params.set('sort_by', 'job_start_date'); params.set('sort_order', startSort.value) }
    else if (dateSort.value) { params.set('sort_by', 'job_updated_at'); params.set('sort_order', dateSort.value) }
    else if (titleSort.value) { params.set('sort_by', 'job_title'); params.set('sort_order', titleSort.value) }
    else if (companySort.value) { params.set('sort_by', 'em_name'); params.set('sort_order', companySort.value) }
    else if (priceSort.value) { params.set('sort_by', 'job_price'); params.set('sort_order', priceSort.value) }
    else { params.set('sort_by', 'job_updated_at'); params.set('sort_order', 'desc') }
...
const filteredJobs = computed(() => {
  ...
  else if (startSort.value) result.sort((a, b) => { const c = new Date(a.job_start_date) - new Date(b.job_start_date); return startSort.value === 'desc' ? -c : c })
...
const resetAllFilters = () => {
  ...
  statusFilter.value = "All"; dateSort.value = ""; startSort.value = ""
  saveFilters()
  page.value = 1
  fetchJobs()
}
