    ...
    <div class="relative z-10 flex w-full max-w-[900px] overflow-hidden rounded-2xl border border-[#e5e7eb] shadow-xl shadow-[#1a1a2e]/10 transition-shadow duration-300 hover:shadow-2xl hover:shadow-[#1a1a2e]/15">
      ...
      <div class="hidden md:flex md:w-[38%] relative flex-col justify-between bg-[#1a1a2e] p-10 overflow-hidden shrink-0">
        ...
        <div>
          <div class="flex items-center gap-2.5 mb-12">
            ...
            <span class="text-white font-bold text-body tracking-tight">Admin Portal</span>
          </div>

          <h1 class="text-white text-3xl font-bold leading-tight mb-3 tracking-tight">
            Create your<br/>account.
          </h1>
          <p class="text-white/40 text-caption leading-relaxed">
        ...
        <div class="flex flex-col gap-2.5">
          <p class="text-white/25 text-meta uppercase tracking-widest mb-0.5">Requirements</p>
          <div v-for="rule in sideRules" :key="rule" class="flex items-start gap-2">
            ...
            <span class="text-white/40 text-meta leading-relaxed">{{ rule }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 bg-white px-10 py-10 flex flex-col justify-center overflow-y-auto">
        ...
        <div class="md:hidden flex items-center gap-2 mb-6">
          ...
          <span class="text-[#1a1a2e] font-bold text-body">Admin Portal</span>
        </div>

        <h2 class="text-[#1a1a2e] text-title font-bold mb-1 tracking-tight">Create account</h2>
        <p class="text-[#666] text-caption mb-6">Fill in your details to register a new admin account.</p>
        ...
        <form @submit.prevent="handleRegister" class="flex flex-col gap-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block mb-1.5 text-meta font-semibold text-[#444] uppercase tracking-wide">Name</label>
              <input
                type="text"
                v-model="name"
                ...
                required
                autocomplete="name"
                class="w-full px-4 py-3 bg-[#fafafa] border border-[#e5e7eb] rounded-xl text-caption text-[#1a1a2e] placeholder:text-[#aaa] focus:outline-none focus:bg-white focus:border-[#1a1a2e] focus:ring-2 focus:ring-[#1a1a2e]/8 hover:border-[#c5c7cb] transition-all"
              />
            </div>
            <div>
              <label class="block mb-1.5 text-meta font-semibold text-[#444] uppercase tracking-wide">Username</label>
              <input
                type="text"
                v-model="username"
                ...
                class="w-full px-4 py-3 bg-[#fafafa] border border-[#e5e7eb] rounded-xl text-caption text-[#1a1a2e] placeholder:text-[#aaa] focus:outline-none focus:bg-white focus:border-[#1a1a2e] focus:ring-2 focus:ring-[#1a1a2e]/8 hover:border-[#c5c7cb] transition-all"
              />
              <p class="mt-1 text-meta text-[#888]">Lowercase, numbers, <code class="bg-[#f0f0f0] px-0.5 rounded">_</code> and <code class="bg-[#f0f0f0] px-0.5 rounded">.</code> only</p>
            </div>
          </div>

          <!-- Email -->
          <div>
            <label class="block mb-1.5 text-meta font-semibold text-[#444] uppercase tracking-wide">Email</label>
            <input
              type="text"
              v-model="email"
              ...
              class="w-full px-4 py-3 bg-[#fafafa] border border-[#e5e7eb] rounded-xl text-caption text-[#1a1a2e] placeholder:text-[#aaa] focus:outline-none focus:bg-white focus:border-[#1a1a2e] focus:ring-2 focus:ring-[#1a1a2e]/8 hover:border-[#c5c7cb] transition-all"
            />
            <p class="mt-1 text-meta text-[#888]">Must end with <code class="bg-[#f0f0f0] px-0.5 rounded">@admin.com</code></p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <!-- Password -->
            <div>
              <label class="block mb-1.5 text-meta font-semibold text-[#444] uppercase tracking-wide">Password</label>
              <div class="flex bg-[#fafafa] rounded-xl border border-[#e5e7eb] overflow-hidden hover:border-[#c5c7cb] focus-within:bg-white focus-within:border-[#1a1a2e] focus-within:ring-2 focus-within:ring-[#1a1a2e]/8 transition-all">
                <input
                  ...
                  class="flex-1 min-w-0 border-0 py-3 px-3 text-caption text-[#1a1a2e] placeholder:text-[#aaa] outline-none bg-transparent"
                />
            ...
            <!-- Confirm -->
            <div>
              <label class="block mb-1.5 text-meta font-semibold text-[#444] uppercase tracking-wide">Confirm</label>
              <div class="flex bg-[#fafafa] rounded-xl border border-[#e5e7eb] overflow-hidden hover:border-[#c5c7cb] focus-within:bg-white focus-within:border-[#1a1a2e] focus-within:ring-2 focus-within:ring-[#1a1a2e]/8 transition-all">
                <input
          ...
          <div v-if="passwordTouched" class="grid grid-cols-2 gap-x-4 gap-y-1.5 bg-[#fafafa] border border-[#e5e7eb] rounded-xl px-4 py-3">
            <div v-for="rule in passwordRules" :key="rule.label" class="flex items-center gap-2">
              ...
              <span class="text-meta" :class="rule.passed ? 'text-[#2e7d32]' : submitted && !rule.passed ? 'text-[#dc3545]' : 'text-[#999]'">{{ rule.label }}</span>
            </div>
          </div>
          ...
          <div v-if="error" class="flex items-center gap-2 bg-[#fff5f5] border border-[#fecaca] rounded-xl px-3.5 py-2.5">
            ...
            <span class="text-[#dc3545] text-caption">{{ error }}</span>
          </div>
          ...
          <div v-if="success" class="flex items-center gap-2 bg-[#f0fdf4] border border-[#bbf7d0] rounded-xl px-3.5 py-2.5">
            ...
            <span class="text-[#2e7d32] text-caption font-medium">{{ success }}</span>
          </div>
          ...
          <button
            type="submit"
            :disabled="loading"
            class="w-full py-3.5 bg-[#1a1a2e] text-white rounded-xl text-caption font-semibold border border-[#1a1a2e] cursor-pointer hover:bg-[#2a2a4e] hover:border-[#2a2a4e] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 mt-1"
          >
        ...
        <p class="mt-5 text-center text-caption text-[#888]">
