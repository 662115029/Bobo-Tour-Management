<template>
  <div>
    ...
    <!-- Delete Modal -->
    <DeleteUserModal :show="showDeleteModal" :name="fl?.fl_name" user-type="Freelancer"
...
const openDocModal = (d) => {
  modalDoc.value = { url: d.file_url, title: formatDocType(d.fl_doc_type), status: d.fl_doc_status, uploadedAt: d.fl_uploaded_at, reviewedBy: d.reviewed_by_name || null, rejectReason: d.reject_reason || null }
}
...
const confirmDelete = async () => {
  if (!fl.value) return
  try {
    const res = await fetch(`${API_BASE}/freelancers/${fl.value.fl_id}`, {
      method: "DELETE",
...
onMounted(async () => {
  ...
  try {
    ...
    availability.value = availData.items || []
